...
public class AchievementSystem : MonoBehaviour
{
    ...
    private GameObject _achievementButton;
    ...
    {
        achievements.Add("Earned_10_gold", new Achievement("십원", 0, 10, false, "10골드 모으기"));
        achievements.Add("Earned_100_gold", new Achievement("백원", 0, 100, false, "100골드 모으기"));
        achievements.Add("Earned_500_gold", new Achievement("오백원", 0, 500, false, "500골드 모으기"));
        
        achievements.Add("Earning_10_gold_per_sec", new Achievement("불로소득", 0, 10, false, "초당 획득 골드 10 달성하기"));
        achievements.Add("Earning_50_gold_per_sec", new Achievement("숨쉬고 돈벌기", 0, 50, false, "초당 획득 골드 50 달성하기"));
        achievements.Add("Earning_150_gold_per_sec", new Achievement("티끌모아 태산", 0, 150, false, "초당 획득 골드 150 달성하기"));
        
        achievements.Add("Use_1_gold", new Achievement("알뜰한 소비", 0, -1, false, "1골드 사용하기"));
        achievements.Add("Use_50_gold", new Achievement("FLEX", 0, -50, false, "50골드 사용하기"));
        
        achievements.Add("Earn_10_gold_at_once", new Achievement("동전 줍기", 0, 10, false, "한번에 10골드 이상 획득"));
        achievements.Add("Earn_200_gold_at_once", new Achievement("로또 5등", 0, 200, false, "한번에 200골드 이상 획득"));

        achievements.Add("Use_skill_3_times", new Achievement("스킬 3회 사용하기", 0, 3, false, "스킬 3회 사용하기"));
    ...
    {
        foreach (var keys in achievements.Keys)
        {
            achievements[keys].LoadAchievement();
        }
    }
    ...
    private void SaveAllAchievements()
    {
        foreach (var keys in achievements.Keys)
        {
            achievements[keys].SaveAchievement();
        }
    }
    ...
    {
        //if (amount >= 10)
            achievements["Earned_10_gold"].TryAchieve(amount);
        
        //if(amount>= 100)
            achievements["Earned_100_gold"].TryAchieve(amount);
        
        //if(amount>= 500)
            achievements["Earned_500_gold"].TryAchieve(amount);
    }

    private void AchieveEarningGoldPerSec(double amount) // GoldPerSec가 일정 수치에 도달하면 달성 가능한 도전과제
    {
        //if (amount >= 10)
            achievements["Earning_10_gold_per_sec"].TryAchieve(amount);
        
        //if (amount >= 50)
            achievements["Earning_50_gold_per_sec"].TryAchieve(amount);
        
        //if (amount >= 150)
            achievements["Earning_150_gold_per_sec"].TryAchieve(amount);
    }

    private void AchieveGoldFluct(double amount) // 한번에 얻거나 소비한 재화의 양에 따른 도전과제 처리
    {
        //if (amount <= -1)
            achievements["Use_1_gold"].TryAchieve(amount);
        
        //if (amount <= -50)
            achievements["Use_50_gold"].TryAchieve(amount);
        
        //if (amount >= 10)
            achievements["Earn_10_gold_at_once"].TryAchieve(amount);
        
        //if (amount >= 200)
            achievements["Earn_200_gold_at_once"].TryAchieve(amount);
    }

    private void AchieveSkillUse() // 스킬 사용에 따른 도전과제 처리
    {
        string achieveName = "Use_skill_3_times";
        var nextProgress = achievements[achieveName].GetProgress() + 1;
        var goal = achievements[achieveName].GetGoal();
        achievements[achieveName].TryAchieve(nextProgress);
    }
    ...
    private IEnumerator ShowAchievementInstant() // 한꺼번에 여러 도전과제 달성 시, 겹치지 않게 큐에 넣어서 순차적으로 보여줌
    {
        if (_achievementDisplayQueue.Count > 0)
        {
            ...
            newInstantText.myText.fontSize = 36;
            newInstantText.myText.color = Color.white;
        }
    ...
    public class Achievement
    {
        ...
        public float GetProgressInPercent() => (float) (_progress / _goal);

        public string GetDescription() => _description;

        public string GetName() => _name;

        public bool GetUnlocked() => _unlocked;
